...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Only process test files instead of process train & valid files.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default='./data',
        help="The directory of dataset. Used if not --only_process_test_file",
    )
    parser.add_argument(
        "--context_path",
        type=Path,
        default='./data',
        help="Context file path. Used if --only_process_test_file",
    )
    parser.add_argument(
        "--test_path",
        type=Path,
        default='./data',
        help="The directory of output dataset. Used if --only_process_test_file",
    )
...
def processTest(args):
    with open(args.context_path, 'r', encoding='utf-8') as fin:
        context = json.load(fin)
    
    with open(args.test_path, 'r', encoding='utf-8') as fin:
        dataset = json.load(fin)
    is_test = True

    with open(args.output_data_dir / f'swag_test.json', 'w', encoding='utf-8') as fout:
        dumpJsonline(preprocess_swag(context, dataset, is_test), fout)
...
if __name__ == '__main__':
    args = parse_args()
    if args.only_process_test_file:
        processTest(args)
    else:        
        processTrainValid(args)